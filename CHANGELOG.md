...
- **我的錢包右上角設定開關**：
  - 在「我的錢包」頁面 (`AccountManager.vue`) 的 Header 右上角，新增了一個精緻的眼睛（Eye）按鈕，點擊後會彈出可愛置中的「顯示類別設定」對話框。
  - 支援勾選要顯示的帳戶類型（現金帳戶、銀行存款、信用卡、電子票證），未勾選的類型將會被隱藏。
  - 此設定會透過 `useAuth` 的 `updateProfileSettings` 自動同步儲存至雲端/本地資料庫（DB）。
- **信用卡設定編輯支援**：
  - 在「編輯帳戶」對話框中，若編輯的帳戶為信用卡類型，會自動展開「信用卡參數設定」的專屬欄位。
  - 支援主人修改該信用卡的「信用額度」、「每月結帳日」與「每月繳款截止日」，點擊儲存後會同步更新至資料庫。
- **全域帳戶顯示過濾 (離線與歷史相容)**：
  - 在 `useLedger.ts` 中新增了 `visibleAccounts` 計算屬性，供需要過濾的頁面使用，而 `accounts` 則保持完整資料，確保歷史記帳明細在帳戶類別被隱藏後依然能正確解析並顯示舊帳戶的名稱（避免出現未知或空白帳戶）。
  - 將以下頁面與元件重構為使用 `visibleAccounts`：
    - **我的錢包**：隱藏的類別將不會顯示於帳戶清單，且類型篩選 Tab 也會動態過濾掉已隱藏的選項。如果當前選擇的篩選 Tab 被隱藏，會自動退回「全部」分頁。
    - **信用卡中心**：若隱藏信用卡類別，則信用卡中心將不會列出任何隱藏的卡片。
    - **記帳表單**：使用者在記帳或編輯交易時，帳戶選擇器中不會出現被隱藏的帳戶，防止誤記。
    - **帳戶選擇器與下拉選單元件**：更新了 `AccountPicker.vue`，使其上方的微型篩選標籤頁（Tab）只顯示主人勾選要顯示的帳戶類型；同時將「我的錢包」中資金互轉彈窗的兩個 `AccountDropdown` 資料來源限制為 `visibleAccounts`，以防已被隱藏的帳戶類別在該處出現。
    - **交易明細**：交易明細中若要編輯某筆交易的帳戶，其下拉選單也會過濾掉被隱藏的帳戶。

## [Web 2.6.9] - 2026-05-31

### 🪄 記帳表單全新推出「快速新增分類」功能與預設圖示庫大升級 (Web 端更新)
- **記帳時隨手新增主/子分類**：
  - 在記帳表單 `TransactionForm.vue` 中，新增了「➕ 新增分類」主分類與「➕ 新增子分類」子分類按鈕，大幅簡化使用者在記帳時發現缺少分類的建立路徑，不再需要特地跳出切換到設定頁面的分類管理。
  - **主分類快速新增 Dialog**：支援輸入分類名稱，並能從豐富的馬卡龍色可愛圖示中選擇，新增成功後自動選中該新分類。**全新重構為與「新增帳戶」一致的 Teleport 置中可愛對話框（Modal）設計，掛載至 #app 最外層，彻底免除底部 TabBar 等層級遮擋；內置垂直自適應滑動與 margin 微調，完美防禦行動端虛擬鍵盤擠壓**。
  - **子分類快速新增 Dialog**：自動鎖定當前選取的主分類，可一鍵快速新增子分類，並在完成後自動選取該新子分類，理財記帳更流暢。**同步升級為 Teleport 掛載與置中 Modal 卡片設計，支援垂直彈性滾動，從根本上徹底杜絕按鈕跑位或不可見的 UX 痛點**。
- **預設分類庫擴充與精美圖示庫升級**：
  - 在 `useAuth.ts` 中，預設支出分類新增「教育學習（GraduationCap）」，包含「學雜費、補習/課程、書籍教科書、文具用品、線上訂閱」等預設子分類，滿足更細緻的日常與成長理財記帳場景。
  - 在 `useLedger.ts` 中，統一提取並擴充 `cuteIconsList` 到 20 個精美圖示（新增畢業帽 🎓、豬豬存錢筒 🐷、金幣 🪙、貓咪 🐱、啞鈴 🏋️、飛機 ✈️、遊戲搖桿 🎮、咖啡 ☕ 等），並實作 `getIconEmoji` 統一管理 emoji 對照表，簡化程式結構，消除 `CategoryManager.vue` 等處原本雜亂的三元運算子 inline 顯示邏輯。

## [Web 2.6.8] - 2026-05-30


### 🎨 優化更新卡片配置與設定頁面體驗 (Web 端更新，支持 Live Update 熱更新)
- **網頁更新卡片移出彩蛋至設定頁面**：
  - 將「📡 逗逗貓自建熱更新監控閣」卡片從「🐾 逗逗貓超高級管理介面（神秘彩蛋）」中移出。
  - 改為直接放置於設定頁面中，位置排列在「📱 原生 Android 系統更新」卡片正上方，方便使用者隨時監控並手動檢查網頁熱更新包。
  - 限制該熱更新監控卡片僅在原生 platform (`v-if="Capacitor.isNativePlatform()"`) 下渲染，以符合 Web 與 Android 原生平台對帳邏輯，在 Web 桌面瀏覽器下自動維持介面精簡。
  - 升級為該卡片套用果凍彈跳（`pop-jelly`）動畫類別，與系統其餘卡片達到 100% Jelly Style 的視覺與微動畫一致性。
