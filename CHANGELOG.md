...
- **帳戶管理卡片圖示水平對齊優化**：為帳戶卡片左側的「自訂 Emoji 頭像」與「預設 SVG 圖示」設計了統一且固定寬高 (`24px`) 的包裹容器 (`.card-icon-wrapper`)，並透過 Flex 置中對齊。這完美解決了兩者原生寬度不同導致後方「帳戶名稱」字體起始起點無法對齊的排版痛點，使帳戶清單在任何螢幕與設備上皆能精準對齊，視覺感受更為高級洗鍊。
