...
- **明細畫面極致體驗優化 (一鍵記帳帶日期 & 📍 快速定位跳轉 & 🐾 回到頂部)**：
  ...
  - **🐾 回到頂部懸浮貓爪按鈕**：當使用者向下滑動明細超過 `300px` 時，右下角會自動浮現一個活潑彈跳的 **「🐾 頂部」** 懸浮貓爪按鈕。點擊可瞬間滑順地返回頁面最上方，讓整體瀏覽與管理體驗極致流暢！
- **手機端拖曳與點擊排序體驗革命 (解決手機上不好按到與不支持的原生痛點)**：
  - **快速點擊排序微調按鈕**：在手機觸控螢幕上原生 Drag & Drop 容易與頁面滾動衝突、按不準且不易微調。我們在 **「每個理財帳戶卡片」**、**「主分類卡片」** 右上角新增了精緻的 **「▲ 上移 / ▼ 下移」** 快速排序按鈕，在 **「子分類 Pill 膠囊」** 內也新增了微型的 **「◀ 往左移 / ▶ 往右移」** 快速排序按鈕！手指只需輕點即可瞬間微調排序，手感極佳、防呆且精準。
  - **手繪風拖曳果凍手把按鈕**：將主分類與帳戶的拖曳把手（`.drag-handle`）進行了物理性的觸控放大，重構成寬高 `26px`、圓角且帶有微陰影與手繪邊框的「精美拖曳把手果凍按鈕」。Hover 變色且觸控極易抓取，徹底解決手機觸控不好按到的痛點，並與整體繪本風完美融合。
  - **子分類觸控熱區物理加大**：子分類膠囊內部的拖曳手把（`.sub-grip`）亦進行了 padding 與 margin 的觸控物理性放大，大幅提高手機上抓取拖曳的靈敏度與舒適度。
- **收支小計欄對齊與防跑版**：
