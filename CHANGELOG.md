...
### 📋 明細列表自訂排序方式
- **四種排序模式**：篩選 Tab 下方新增排序列，提供「📅 最新」「📅 最舊」「💰 高→低」「💰 低→高」四顆排序小按鈕，預設依最新日期排列。
- **即時切換**：點擊排序按鈕後列表立即重新排列，不影響現有的類型篩選（全部 / 支出 / 收入 / 轉帳）與關鍵字搜尋的組合。
- **Q 彈按鈕樣式**：排序按鈕套用 `.btn-jelly` 果凍動畫，與整體 UI 風格一致；已選中按鈕以深色邊框 + 粗體標示。
