...
- **細分對帳狀態**：在 `Settings.vue` 的 App 原生更新檢查中，新增了狀態識別。能精準區分「真正的網路連線/API 請求失敗（如 fetch 失敗或 403 頻繁請求）」與「成功連線但遠端無可用 APK 檔案（404 或 releases 無 APK）」的狀態，告別一律拋出「遠端沒有可安裝的版本」的對帳失敗混淆提示。
