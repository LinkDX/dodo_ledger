...
3. **📂 嚴謹雙層分類與級聯編輯**：支援「主分類 ➜ 子分類」的雙層分類結構，並首創**智慧級聯更新 (Cascading Update)** 功能！當您原地編輯或更新主/子分類名稱與 icon 時，歷史明細與週期記帳會自動在背景同步更新，確保過往明細帳目永不失效。
4. **📊 每日彙整與極致排版**：提供「日區塊逐項展開」與「每日彙整卡片」雙檢視模式，預設首選極簡的每日彙整卡片。單筆交易項目完美繼承正圓形 42px 大 icon 與清晰大字體，徹底剔除了無趣的原生 HTML `<select>` 下拉選單，重構為 100% 自訂的馬卡龍色果凍風下拉選單！
5. **💳 信用卡完整生命週期**：支援「額度當下全扣，分月攤還」嚴謹分期邏輯、未出帳/已出帳單自動歸屬、結帳日繳款日管理，以及一鍵扣繳連動還款交易。信用卡繳清後，明細會自動標記亮麗的「✓ 已繳清」馬卡龍綠 jelly 標籤。
6. **👁️ 淨資產隱藏保護**：首頁資產看板支援一鍵「眼睛」開關 (Eye / EyeOff)，隱藏時所有敏感帳戶餘額、資產與負債將安全顯示為 `***`。
7. **🔄 雙資料存取模式**：預設免登入、免填寫金鑰，直接在 LocalStorage 體驗完整功能，逗逗貓貼身相伴；隨時可在設定中輸入 Firebase 設定一鍵備份升級至雲端。
8. **🛡️ 業界級 DevOps 機制**：內建 Vitest 單元測試，配置 GitHub Actions CI/CD，發布到 GitHub Pages 前會進行 100% 自動化測試與 TypeScript 編譯驗證。
